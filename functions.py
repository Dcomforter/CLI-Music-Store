...
def userMessage():
    ...
    print("checkout\t\t-\t Checkout all items from your current shopping cart")
    ...
    while logged_in:
        ...
        if command[0] == "view":
            try:
                if command[1] == "all":
                    ...
                    with open("albums.csv") as f:
                        csv_f = csv.reader(f)
                        for row in csv_f:
                            print('{:<15} {:<16} {:<16} {:<15} {:<15}'.format(*row))
                    print()                    
            ...
            try:
                ...
                if album_quantity == 0:
                    print("\nAlbum not found or sold out. Kindly VIEW ALL to see a list of available items. \n")
                else:
                    user_quantity = int(input("\nEnter quantity: "))
                    # Check if the quantity does not exceed the inventory
...
def rmCart(user, item):
    with open('carts.csv') as readFile: # Read the file
        ...
        for row in reader:
            ...
            else: # Create a backup file to save the contents without the searched item
    ...
    with open('albums.csv') as rf: # Read the file
        ...
        for r in readerf:
            if r[0] == id: # edit the inventory
                ...
                new_quantity = r[4]
            # Create a backup file to save the contents without the searched item
